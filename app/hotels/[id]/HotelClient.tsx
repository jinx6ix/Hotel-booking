...
const hotelCoordinates: Record<string, { lat: number; lng: number }> = {
  "nairobi-001": { lat: -1.2921, lng: 36.8219 },
  "nairobi-002": { lat: -1.2921, lng: 36.8219 },
  "nairobi-003": { lat: -1.2921, lng: 36.8219 },
  "nairobi-004": { lat: -1.2921, lng: 36.8219 },
  "nairobi-005": { lat: -1.2921, lng: 36.8219 },
  "nairobi-006": { lat: -1.2921, lng: 36.8219 },
  "nairobi-007": { lat: -1.2921, lng: 36.8219 },
  "nairobi-008": { lat: -1.2921, lng: 36.8219 },
  "nairobi-009": { lat: -1.2921, lng: 36.8219 },
  "nairobi-010": { lat: -1.2921, lng: 36.8219 },
  "nairobi-011": { lat: -1.2921, lng: 36.8219 },
  "nairobi-014": { lat: -1.2921, lng: 36.8219 },
  "nairobi-015": { lat: -1.2921, lng: 36.8219 },
  "naivasha-001": { lat: -0.7176, lng: 36.4311 },
  "naivasha-002": { lat: -0.7176, lng: 36.4311 },
  "naivasha-003": { lat: -0.7176, lng: 36.4311 },
  "naivasha-004": { lat: -0.7176, lng: 36.4311 },
  "naivasha-005": { lat: -0.7176, lng: 36.4311 },
  "naivasha-006": { lat: -0.7176, lng: 36.4311 },
  "naivasha-007": { lat: -0.7176, lng: 36.4311 },
  "nakuru-001": { lat: -0.3031, lng: 36.0800 },
  "nakuru-002": { lat: -0.3031, lng: 36.0800 },
  "nakuru-003": { lat: -0.3031, lng: 36.0800 },
  "nakuru-012": { lat: -0.3031, lng: 36.0800 },
  "nakuru-013": { lat: -0.3031, lng: 36.0800 },
  "kisumu-001": { lat: -0.1022, lng: 34.7617 },
  "mombasa-001": { lat: -4.0435, lng: 39.6682 },
  "maasai-mara-010": { lat: -1.4894, lng: 35.1382 },
  "amboseli-010": { lat: -2.6458, lng: 37.2536 },
};
...
function HotelVideo({ videoUrl, videoEmbedUrl, hotelName, hotelImage }: HotelVideoProps) {
  ...
  const getVimeoId = (url: string) => {
    const regExp = /vimeo\.com\/(?:channels\/|groups\/[^\/]*\/|video\/|)(\d+)/;
    const match = url.match(regExp);
      ...
      {!showVideo ? (
        ...
        >
          ...
          <div className="absolute bottom-4 left-4 right-4 text-white text-center">
            <p className="text-lg font-semibold">Watch Hotel Video Tour</p>
            <p className="text-sm opacity-90">Experience {hotelName} in action</p>
          </div>
        </div>
      ) : (
...
function AccessibilitySection({ accessibilityFeatures, accessibilityImages, hotelName }: AccessibilitySectionProps) {
  const [activeCategory, setActiveCategory] = useState<string>("all");
  const [showAllImages, setShowAllImages] = useState(false);

  if (!accessibilityFeatures || accessibilityFeatures.length === 0) return null;
    ...
    <div className="mb-12 bg-white rounded-2xl shadow-lg overflow-hidden">
      <div className="bg-blue-600 p-6">
        <div className="flex items-center gap-3 text-white">
          ...
          <div>
            <h3 className="text-2xl font-bold">Accessibility Features</h3>
            <p className="text-blue-100">Detailed information about accessible facilities at {hotelName}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredFeatures.map((feature) => (
            <div key={feature.id} className="border rounded-xl p-5 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-3">
                <h4 className="text-lg font-bold text-gray-900">{feature.title}</h4>
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(feature.status)}`}>
                  {feature.status === 'available' ? 'Available' : feature.status === 'partial' ? 'Partial' : 'Unavailable'}
                </span>
              </div>
              <p className="text-gray-600 mb-3">{feature.description}</p>
              {feature.measurements && (
                <p className="text-sm text-gray-500 mb-3 font-mono">{feature.measurements}</p>
              )}
              {feature.details?.items && (
                <div className="mt-3 space-y-2">
                  {feature.details.items.map((item, idx) => (
                    <div key={idx} className="flex items-start gap-2 text-sm">
                      <span className={`inline-block w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${
      ...
      {accessibilityImages && Object.keys(accessibilityImages).length > 0 && (
        <div className="border-t p-6">
          <h4 className="text-xl font-bold text-gray-900 mb-4">Accessibility Photos</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(accessibilityImages).slice(0, showAllImages ? undefined : 4).map(([category, images]) => 
              images?.slice(0, 2).map((img, idx) => (
                <div key={`${category}-${idx}`} className="relative aspect-square rounded-lg overflow-hidden cursor-pointer group">
                  <Image
                    src={img}
                    alt={`${category} accessibility feature at ${hotelName}`}
                    fill
          ...
          {Object.keys(accessibilityImages).length > 4 && (
            <button
              onClick={() => setShowAllImages(!showAllImages)}
              className="mt-4 text-blue-600 font-semibold hover:text-blue-700 transition-colors"
            >
              {showAllImages ? 'Show Less' : `View All ${Object.keys(accessibilityImages).length} Accessibility Photos`}
            </button>
          )}
        </div>
      )}
...
export default function HotelClient({ hotel, location }: HotelClientProps) {
  ...
  const getFilteredRooms = () => {
    if (roomFilter === "accessible") {
      return hotel.accessible ? hotel.rooms.filter(room => room.accessible === true) : [];
    }
  ...
  const hasAccessibleRooms = hotel.accessible === true && hotel.rooms.some(room => room.accessible === true);
  ...
  const seoSections = [
    {
      ...
      content: `${hotel.name} is a premium ${hotel.rating}-star hotel located in the heart of ${hotel.location}, Kenya. Offering luxury accommodations with world-class amenities, this hotel combines modern comfort with authentic Kenyan hospitality. Whether you're visiting for business or leisure, ${hotel.name} provides the perfect base for exploring ${hotel.location} and its surrounding attractions.${hotel.accessible ? ' The hotel features wheelchair accessible rooms with roll-in showers, wide doorways, and grab bars to ensure a comfortable stay for all guests.' : ''}`
    },
    {
      title: "Location & Accessibility",
      content: `Strategically located in ${hotel.location}, ${hotel.name} offers easy access to major attractions, business districts, and transportation hubs. The hotel's prime location makes it ideal for both tourists exploring Kenya's natural wonders and business travelers attending meetings in the city center.${hotel.accessible ? ' The property is wheelchair accessible with designated parking, accessible pathways, and elevator access to all floors.' : ''}`
    },
    {
    ...
    {
      title: "Room Features",
      content: `Each room at ${hotel.name} is designed with comfort and luxury in mind. Features include premium bedding, high-speed internet, modern bathrooms, and stunning views. ${hasAccessibleRooms ? 'Our accessible rooms are specially designed to accommodate guests with mobility needs, featuring wider doorways, roll-in showers, grab bars, and lowered fixtures.' : ''}`
    }
  ];

  const reviews = [
    { name: "John D.", rating: 5, text: "Exceptional service and beautiful property. The staff went above and beyond to make our stay memorable.", date: "2024-03-15", location: "London, UK", verified: true },
    { name: "Sarah M.", rating: 4, text: "Great location, clean rooms, and friendly staff. The breakfast buffet was amazing with local and international options.", date: "2024-03-10", location: "New York, USA", verified: true },
    { name: "Mike L.", rating: 5, text: "Best hotel experience in Kenya! The safari package was well organized and the guides were extremely knowledgeable.", date: "2024-02-28", location: "Sydney, Australia", verified: true },
    { name: "Emma K.", rating: 4, text: "Comfortable beds, excellent room service, and beautiful pool area. Would definitely recommend for families.", date: "2024-02-20", location: "Berlin, Germany", verified: true },
  ];
  ...
  return (
    <>
          ...
          <div className="flex space-x-8">
            ...
            {hasAccessibleRooms && (
              ...
              >
                ...
                Accessible Rooms ({hotel.accessibleRoomCount || 0})
              </button>
            )}
          </div>
        </div>
      ...
      <nav className="bg-white border-b sticky top-14 z-40 shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex space-x-8 overflow-x-auto">
            ...
            {hotel.accessible && hotel.accessibilityFeatures && (
              <button
                onClick={() => scrollToSection("accessibility")}
                className={`py-4 px-2 border-b-2 font-medium text-sm capitalize whitespace-nowrap transition-colors flex items-center gap-2 ${
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <h1 className="text-3xl md:text-4xl font-bold text-gray-900">{hotel.name}</h1>
                {hotel.accessible && (
              ...
              <div className="flex items-center gap-4 text-gray-600">
                ...
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={18} className={i < Math.floor(hotel.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"} />
                  ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[hotel.image, ...hotel.gallery].slice(0, 7).map((img, idx) => (
              <div
                key={idx}
                className={`bg-gray-200 rounded-xl overflow-hidden cursor-pointer transition-all duration-300 hover:scale-[1.02] hover:shadow-xl ${idx === 0 ? "col-span-2 row-span-2" : ""}`}
                onClick={() => {
                  setGalleryIndex(idx);
                  setShowGallery(true);
                }}
              >
                <div className="relative aspect-square">
                  <Image src={img || "/placeholder.svg"} alt={`${hotel.name} - ${hotel.location} hotel view ${idx + 1}`} fill className="object-cover" sizes="(max-width: 768px) 50vw, 25vw" loading={idx < 2 ? "eager" : "lazy"} />
                </div>
              </div>
            ))}
            <button onClick={() => setShowGallery(true)} className="col-span-2 md:col-span-1 bg-linear-to-r from-orange-500 to-amber-500 text-white rounded-xl flex flex-col items-center justify-center font-semibold hover:from-orange-600 hover:to-amber-600 transition-all duration-300 p-6">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="space-y-5">
                {hotel.amenities.slice(0, 6).map((amenity, i) => (
                  <div key={i} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg hover:bg-orange-50 transition-colors">
                    <div className="shrink-0">{amenityIcons[amenity] || <Wifi className="text-gray-600" size={24} />}</div>
                    <span className="font-medium text-gray-800">{amenity}</span>
                  </div>
                ))}
              </div>
              <button onClick={() => setShowMoreFacilities(true)} className="mt-6 text-orange-600 font-semibold hover:text-orange-700 transition-colors flex items-center gap-2">
                View All {hotel.amenities.length} Facilities <ChevronRight size={18} />
              </button>
            </div>
            ...
            <div className="bg-linear-to-br from-orange-500 to-amber-500 p-8 rounded-2xl shadow-lg hover:shadow-xl transition-shadow">
              ...
              <button onClick={() => setShowMap(true)} className="mt-6 bg-white text-orange-600 px-6 py-3 rounded-lg font-semibold hover:bg-gray-100 transition-colors w-full">
                <span className="flex items-center justify-center gap-2"><MapPin size={20} /> Show Interactive Map</span>
              </button>
            </div>
            ...
            <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-lg hover:shadow-xl transition-shadow">
              ...
              <div className="space-y-4 mb-6">
                <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <a href={`tel:${hotel.phone}`} className="text-blue-600 hover:text-blue-700">{hotel.phone}</a>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-green-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <a href={`mailto:${hotel.email}`} className="text-green-600 hover:text-green-700">{hotel.email}</a>
                  </div>
                </div>
              </div>
              <div className="border-t pt-6">
                <div className="flex items-center gap-2 mb-4">
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} size={20} className={i < Math.floor(hotel.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"} />
                    ))}
                  </div>
                  <span className="text-lg font-bold">{hotel.rating}/5</span>
                  <span className="text-gray-500">({reviews.length} reviews)</span>
                </div>
                <button onClick={() => scrollToSection("reviews")} className="text-orange-600 font-semibold hover:text-orange-700 transition-colors flex items-center gap-2">
                  Read Guest Reviews <ChevronRight size={18} />
                </button>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                {hasAccessibleRooms && (
                  <div className="flex justify-between py-3 border-b">
                    ...
                    <span className="font-semibold text-blue-600">{hotel.rooms.filter(r => r.accessible).length} Available</span>
                  </div>
                )}
                <div className="flex justify-between py-3 border-b">
                ...
                {hotel.accessible && (
                  <div className="flex justify-between py-3 border-b">
                    ...
                    <span className="font-semibold text-green-600 flex items-center gap-1"><Accessibility size={16} /> Wheelchair Accessible</span>
                  </div>
                )}
              </div>
            </div>
      ...
      {hotel.accessible && hotel.accessibilityFeatures && (
        <section ref={sectionRefs.accessibility} className="py-16 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <AccessibilitySection 
              accessibilityFeatures={hotel.accessibilityFeatures}
              accessibilityImages={hotel.accessibilityImages}
              hotelName={hotel.name}
            />
          </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center mb-10">
            <h2 className="text-3xl font-bold text-gray-900">
              {roomFilter === "accessible" ? "Accessible Rooms" : "Standard & Deluxe Rooms"}
            </h2>
          ...
          {filteredRooms.length > 0 ? (
                ...
                <div key={room.id} className="bg-white rounded-2xl shadow-lg overflow-hidden">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 p-8">
                    <div className="lg:col-span-1">
                      <RoomImageCarousel images={room.images} roomType={room.type} className="h-80 lg:h-full" />
                    </div>
                      ...
                      <div className="flex flex-col h-full">
                        <div className="flex-1">
                          <div className="flex items-start justify-between mb-4">
                            ...
                            {room.accessible && (
                              <span className="bg-blue-100 text-blue-700 text-sm px-4 py-2 rounded-full font-medium flex items-center gap-2">
                                <Accessibility size={16} /> Wheelchair Accessible
                              </span>
                            )}
                          </div>
                          ...
                          {room.accessible && room.accessibilityFeatures && (
                            <div className="mb-6 p-4 bg-blue-50 rounded-xl border border-blue-200">
                              <h4 className="font-semibold text-blue-800 mb-2 flex items-center gap-2">
                                <Accessibility size={18} /> Accessibility Features
                              </h4>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            ...
                            <div className="text-center p-3 bg-gray-50 rounded-lg">
                              ...
                              <div className="text-xl font-bold text-gray-900">{room.maxOccupancy <= 2 ? "King Bed" : "Multiple Beds"}</div>
                            </div>
                            <div className="text-center p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6 border-t">
                          ...
                          <div className="flex gap-3">
                            ...
                            <button
                              onClick={() => alert(`More details about ${room.type}`)}
            ...
            <div className="text-center py-12 bg-white rounded-2xl">
              <div className="max-w-md mx-auto">
                <div className="text-gray-400 mb-4"><Bed size={64} className="mx-auto" /></div>
                <h3 className="text-xl font-semibold text-gray-700 mb-2">No {roomFilter === "accessible" ? "accessible" : "standard"} rooms available</h3>
                <p className="text-gray-500 mb-6">
                  {roomFilter === "accessible" 
                    ? "This hotel does not currently have accessible rooms available. Please check back later or contact us directly for assistance."
                    : "Please check back later or contact us directly for availability."}
                </p>
                {roomFilter === "accessible" && (
                  <button onClick={() => setRoomFilter("normal")} className="text-orange-600 font-semibold hover:text-orange-700 transition-colors">
                    View Standard Rooms Instead
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {hotel.amenities.map((amenity, index) => (
              <div key={index} className="bg-gray-50 p-6 rounded-xl hover:bg-orange-50 transition-colors group border border-gray-200">
                <div className="flex flex-col items-center text-center">
                  ...
                  <span className="font-medium text-gray-800 group-hover:text-gray-900">{amenity}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="bg-white p-8 rounded-2xl shadow-lg mb-8">
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div><p className="font-semibold text-gray-800">Address</p><p className="text-gray-600">{hotel.address}</p></div>
                  </div>
                  <div className="flex items-start gap-3">
                    <Car className="text-blue-500 mt-1" size={20} />
                    <div><p className="font-semibold text-gray-800">Transportation</p><p className="text-gray-600">Easy access to public transport and major highways</p></div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-lg">
              <div className="h-[500px] rounded-xl overflow-hidden">
                <MapWithNoSSR center={hotelCoords} zoom={15} markers={[{ position: hotelCoords, title: hotel.name }]} className="w-full h-full rounded-lg" />
              </div>
              <div className="mt-6 space-y-3">
                <button onClick={() => setShowMap(true)} className="w-full bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-lg font-semibold transition-colors flex items-center justify-center gap-2">
                  <Globe size={20} /> Open Full Screen Map
                </button>
                <a href={`https://maps.google.com/?q=${hotelCoords.lat},${hotelCoords.lng}`} target="_blank" rel="noopener noreferrer" className="block text-center text-orange-600 font-semibold hover:text-orange-700 transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {policies.map((policy, index) => (
              <div key={index} className="flex items-start gap-4 p-6 bg-gray-50 rounded-xl hover:bg-orange-50 transition-colors">
                <div className="text-orange-500 shrink-0">{policy.icon}</div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex items-center gap-2">
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={24} className={i < Math.floor(hotel.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"} />
                ))}
              </div>
              <span className="text-2xl font-bold">{hotel.rating}</span><span className="text-gray-500">/5</span>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {reviews.map((review, index) => (
              <div key={index} className="bg-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition-shadow">
                <div className="flex justify-between items-start mb-4">
                  ...
                  {review.verified && <span className="bg-green-100 text-green-800 text-xs px-3 py-1 rounded-full font-medium">Verified Stay</span>}
                </div>
      ...
      {showGallery && (
        <div className="fixed inset-0 bg-black/95 z-100 flex items-center justify-center p-4">
          <button onClick={() => setShowGallery(false)} className="absolute top-4 right-4 text-white hover:text-gray-300 bg-black/50 p-2 rounded-full"><X size={32} /></button>
          <button onClick={() => setGalleryIndex((prev) => (prev - 1 + hotel.gallery.length + 1) % (hotel.gallery.length + 1))} className="absolute left-4 text-white hover:text-gray-300 bg-black/50 p-3 rounded-full"><ChevronLeft size={32} /></button>
          <div className="relative max-w-7xl max-h-[90vh]">
            <Image src={galleryIndex === 0 ? hotel.image : hotel.gallery[galleryIndex - 1]} alt={`${hotel.name} gallery ${galleryIndex + 1}`} width={1200} height={800} className="max-w-full max-h-[80vh] object-contain" priority />
          </div>
          <button onClick={() => setGalleryIndex((prev) => (prev + 1) % (hotel.gallery.length + 1))} className="absolute right-4 text-white hover:text-gray-300 bg-black/50 p-3 rounded-full"><ChevronRight size={32} /></button>
          <p className="absolute bottom-8 left-1/2 transform -translate-x-1/2 text-white text-lg">{galleryIndex + 1} / {hotel.gallery.length + 1}</p>
        </div>
      )}
      ...
      {showMap && (
        <div className="fixed inset-0 bg-black/95 z-100 flex flex-col">
          <div className="bg-white p-4 flex justify-between items-center">
            ...
            <button onClick={() => setShowMap(false)} className="p-2 hover:bg-gray-100 rounded-lg"><X size={24} /></button>
          </div>
          <div className="flex-1 relative">
            <MapWithNoSSR center={hotelCoords} zoom={16} markers={[{ position: hotelCoords, title: hotel.name }]} className="w-full h-full" />
          </div>
          <div className="bg-white p-6">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="flex gap-4">
                <a href={`https://maps.google.com/?q=${hotelCoords.lat},${hotelCoords.lng}`} target="_blank" rel="noopener noreferrer" className="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-lg font-semibold text-center">Google Maps</a>
                <a href={`https://maps.apple.com/?q=${hotelCoords.lat},${hotelCoords.lng}`} target="_blank" rel="noopener noreferrer" className="flex-1 bg-black hover:bg-gray-800 text-white py-3 rounded-lg font-semibold text-center">Apple Maps</a>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/75 z-100 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => setShowMoreFacilities(false)} className="p-2 hover:bg-gray-100 rounded-lg"><X size={24} /></button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {hotel.amenities.map((amenity, i) => (
                <div key={i} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl hover:bg-orange-50 transition-colors">
                  <div className="text-gray-600">{amenityIcons[amenity] || <Wifi size={24} />}</div>
                  <span className="font-medium text-gray-800">{amenity}</span>
                </div>
              ))}
            </div>
      ...
      {showBookingModal && (
        <div className="fixed inset-0 bg-black/75 z-100 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              <div><h3 className="text-3xl font-bold text-gray-900">Book {hotel.name}</h3><p className="text-gray-600">Complete the form below to check availability</p></div>
              <button onClick={() => setShowBookingModal(false)} className="p-2 hover:bg-gray-100 rounded-lg"><X size={24} /></button>
            </div>
            <form onSubmit={handleBookingSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div><label className="block text-sm font-medium text-gray-700 mb-2">Check-in Date</label><input type="date" value={bookingForm.checkIn} onChange={(e) => setBookingForm({...bookingForm, checkIn: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" required min={today} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-2">Check-out Date</label><input type="date" value={bookingForm.checkOut} onChange={(e) => setBookingForm({...bookingForm, checkOut: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" required min={bookingForm.checkIn || tomorrow} /></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className="fixed bottom-8 right-8 z-50">
        <button onClick={() => setShowBookingModal(true)} className="bg-linear-to-r from-orange-500 to-red-500 text-white px-6 py-4 rounded-full font-bold text-lg shadow-2xl hover:shadow-3xl transition-all hover:scale-105">
